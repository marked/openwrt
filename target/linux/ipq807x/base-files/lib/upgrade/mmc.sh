...
mmc_do_flash() {
	...
	# make sure its unbound
	losetup --detach-all
	mount | grep loop && {
		echo "Failed to unmount all loop devices. Skip this try."
		sleep 10
		reboot -f
	}
	...
	if [[ $(type -P "losetup") ]]; then
		...
		echo "Warning: missing losetup.  Skipping backup to rootfs_data"
	fi
